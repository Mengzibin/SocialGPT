...
def get_story(dir):
    for root,_,files in os.walk(dir):
        ...
        for file in files:
            ...
            for i in range(len(txt)):
                string += txt[i]
            txt_dict[file.split('.')[0].split('_')[0]+'_'+file.split('.')[0].split('_')[1]] = string
            F.close()
    return txt_dict
...
def prompt_relation(example,relationship,prelude,question,system_prompt):
    prompt =  prelude+'\n\n'+relationship+'\n\n'+'****************************************************************\nExample:\n'+example+'\n****************************************************************\n\n[1. image description]:\n'+question['story']+'\n\n[2. Question]: \nWhat are the most likely social relationships between '+question['R1']+' and '+question['R2']+'? Choose only one from {<father-child>, <mother-child>, <grandpa-grandchild>, <grandma-grandchild>, <friends>, <siblings>, <classmates>, <lovers/spouses>, <presenter-audience>, <teacher-student>, <trainer-trainee>, <leader-subordinate>, <band members>, <dance team members>, <sport team members>, <colleagues>}..\n\n[3. Answer]:'
    a = get_conversation_template('vicuna-13b-v1.5')
    a.append_message(a.roles[0], f"{prompt}")
    a.append_message(a.roles[1], f"{system_prompt}")
    answer = a.get_prompt()
    print("\n\n\n"+answer)
    return prompt
