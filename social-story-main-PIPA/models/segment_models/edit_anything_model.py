...
class EditAnything:
    ...
    def region_classify_w_blip2_prompt(self, images, prompt):
        inputs = self.image_caption_model.processor(images=images, text=prompt, return_tensors="pt").to(self.device, self.data_type)
        generated_ids = self.image_caption_model.model.generate(**inputs)
        generated_texts = self.image_caption_model.processor.batch_decode(generated_ids, skip_special_tokens=True)
        return [text.strip() for text in generated_texts]
    ...
    def region_level_semantic_api(self, image, anns, topk=10):
        """
        ...
        if len(anns) == 0:
            return []
        #sorted_anns = sorted(anns, key=(lambda x: x['area']), reverse=True) #predicted_iou
        # topk_anns = sorted_anns[:min(topk, len(sorted_anns))]
        with ThreadPoolExecutor() as executor:
            regions_and_anns = list(executor.map(lambda ann: self.process_ann(ann, image), anns))
    ...
    def region_level_semantic_api_prompt(self, image, anns, prompt, topk=10):
        """
        rank regions by area, and classify each region with blip2, parallel processing for speed up
        Args:
            image: numpy array
        ...
        # topk_anns = sorted_anns[:min(topk, len(sorted_anns))]
        with ThreadPoolExecutor() as executor:
            regions_and_anns = list(executor.map(lambda ann: self.process_ann(ann, image), sorted_anns))
        regions = [region for region, _ in regions_and_anns]
        print('..........')
        region_class_labels = self.region_classify_w_blip2_prompt(regions, prompt)
        for (region, ann), class_label in zip(regions_and_anns, region_class_labels):
            ann['class_name'] = class_label
        end_time = time.time()
        print("region_level_semantic_api took {:.2f} seconds".format(end_time - start_time))
    ...
    def semantic_class_w_mask_prompt(self, img_src, anns, prompt):
        image = Image.open(img_src)
        image = resize_long_edge(image, 384)
        return self.region_level_semantic_api_prompt(image, anns, prompt)